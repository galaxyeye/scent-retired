 ...
 * {@code clean} methods in {@link org.qiwur.scent.jsoup.Jsoup}.
 */
public class Cleaner {
   ...
   * modified. Only elements from the dirt document's <code>body</code> are
   * used.
   * 
   * @param dirtyDocument
  ...
  private final class CleaningVisitor implements NodeVisitor {
    ...
    public void head(Node source, int depth) {
      if (source instanceof Element) {
        ...
        if (whitelist.isSafeTag(sourceEl.tagName())) { // safe, clone and copy
                                                       // safe attrs
